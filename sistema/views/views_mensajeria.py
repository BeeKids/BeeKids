...
@login_required
def selectorMensajeria(request):
    usuarios = UsuarioEscolar.objects.exclude(id=request.user.id).order_by("username")
    grupos = Grupo.objects.all().order_by("nombre")
    return render(request, "sistema/Vista_MensajeriaSelector.html", {
        "usuarios": usuarios,
        "grupos": grupos
    })
